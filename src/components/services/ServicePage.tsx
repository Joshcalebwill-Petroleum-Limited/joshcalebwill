...
function GalleryImage({
  ...
  if (failed) {
    return (
      <div
        className={cn(
          "bg-background-soft flex items-center justify-center",
          className
        )}
      >
...
const sectionStyles: Record<string, string> = {
  ...
  brand: "bg-gradient-to-r from-orange via-red to-yellow text-white",
};
  ...
  return (
    <div className="bg-background overflow-x-hidden">
            ...
            {gallery.length > 4 && (
              <div className="mt-3 sm:mt-4 flex justify-center gap-3 sm:gap-4">
                {gallery.slice(4).map((src, i) => (
                  <Reveal key={src + i} delay={0.2 + i * 0.06}>
                    <motion.div
                      whileHover={{ y: -4 }}
                      className="relative aspect-[4/3] w-[42vw] max-w-[220px] overflow-hidden rounded-2xl shadow-md sm:w-[28vw] sm:max-w-[260px]"
                    >
                      <GalleryImage src={src} alt="" />
                    </motion.div>
                  </Reveal>
      ...
      {service.sections?.map((section, i) => {
        const variant = section.variant ?? (i % 2 === 0 ? "warm" : "brand");
        ...
        return (
          <section
            ...
            className={cn(
              ...
              sectionStyles[variant] ?? sectionStyles.warm
            )}
          >
            <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
              <div
                className={cn(
                  "grid gap-8 lg:grid-cols-12 lg:gap-12 items-start",
              ...
              >
                  ...
                  {section.subtitle && (
                    <p
                      className={cn(
                        ...
                        isBrand ? "text-white/80" : "text-orange"
                      )}
                    >
                      {section.subtitle}
                    </p>
                ...
                <Reveal delay={0.08} className="lg:col-span-7 space-y-4">
                  {section.paragraphs.map((p, pi) => (
                    <p
                      key={pi}
                      className={cn(
                        "text-base leading-relaxed sm:text-lg",
                        isBrand ? "text-white/90" : "text-foreground-muted"
                      )}
                    >
                      {p}
                    </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {service.capabilities.map((item, i) => (
              <Reveal key={item} delay={i * 0.05}>
                <div className="flex h-full items-start gap-3 rounded-2xl border border-border bg-white px-5 py-4 transition-all hover:border-orange/30 hover:shadow-md">
                  <CheckCircle2
                    className={cn(
            ...
            {related.map((s, i) => (
              <Reveal key={s.slug} delay={i * 0.07}>
                <Link
                  ...
                  className="group flex h-full flex-col rounded-2xl border border-border bg-background-elevated p-6 transition-all hover:-translate-y-1 hover:shadow-lg hover:border-orange/30"
                >
                  <div
                    className={cn(
